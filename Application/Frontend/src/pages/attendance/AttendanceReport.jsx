...
export default function AttendanceReport() {
  ...
  useEffect(() => {
    ...
    setFilteredData(
      reportData.filter((s) => {
        ...
        return (
          name.includes(studentFilter.toLowerCase()) ||
  ...
  const lowAttendance = [...filteredData]
    .filter((s) => s.percent < 75 && s.total > 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-between items-start gap-3 mb-7">
          <div>
            <h1 className="text-2xl font-black text-slate-800 font-serif">
              Attendance Reports
            </h1>
            <p className="text-slate-500 text-sm mt-0.5">
              Monthly analytics and insights on student attendance
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={exportCSV}
              className="flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold rounded border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition duration-150 cursor-pointer"
            >
              CSV
            </button>
            <button
              onClick={exportExcel}
              className="flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold rounded border border-green-200 bg-green-50 text-green-700 hover:bg-green-100 transition duration-150 cursor-pointer"
            >
              Excel
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {statCards.map(({ label, value, color, icon, bg }) => (
            <div
              key={label}
              className="bg-white rounded-md p-4 border border-slate-100/80 shadow-sm flex justify-between items-center h-22"
            >
              <div>
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                  {label}
                </span>
                <span className={`text-2xl font-black ${color} mt-1 block`}>
                  {value}
                </span>
              </div>
              <div
                className={`w-10 h-10 rounded ${bg} flex items-center justify-center shrink-0`}
              >
                {icon}
              </div>
            </div>
        ...
        <div className="bg-white rounded-md shadow-sm border border-slate-100 p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
            <input
              ...
              className="w-full px-3 py-2 text-xs border border-slate-200 rounded bg-slate-50 focus:border-indigo-400 focus:bg-white outline-none transition font-semibold text-slate-700"
            />
            <select
              value={sectionFilter}
              onChange={(e) => setSectionFilter(e.target.value)}
              className="w-full px-3 py-2 text-xs border border-slate-200 rounded bg-white outline-none focus:ring-2 focus:ring-indigo-400 font-semibold text-slate-700 cursor-pointer"
            >
        ...
        {/* Main Table */}
        <div className="bg-white rounded-md overflow-hidden shadow-sm border border-slate-100/80 mb-6">
                      ...
                      return (
                        ...
                        >
                          <td className="px-4 py-3 text-xs">
                            <span className="bg-slate-100 px-2 py-0.5 rounded-md text-[10px] font-mono text-slate-500 font-bold border border-slate-200/50">
                              {s.rollNumber || "—"}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded flex items-center justify-center text-white font-bold text-xs ${avatarColors[i % avatarColors.length]}`}
                              >
                          ...
                          <td className="px-4 py-3 text-xs text-blue-600 font-bold">
                            {s.late}
                          </td>
                          <td className="px-4 py-3">
                ...
                {filteredData.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Stat Breakdown Grid */}
                      <div className="grid grid-cols-4 gap-2 bg-slate-50 p-2.5 rounded border border-slate-100/80 text-center">
                        <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Top Performers */}
          <div className="bg-white rounded-md overflow-hidden shadow-sm border border-slate-100/80">
            ...
            <div className="divide-y divide-slate-100">
              {topPerformers.length ? (
                topPerformers.map((s, i) => (
                  <div
                    key={s._id}
                    className="flex justify-between items-center px-5 py-3 hover:bg-emerald-50/10 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded flex items-center justify-center text-white font-bold text-xs ${avatarColors[i % avatarColors.length]}`}
            ...
            <div className="divide-y divide-slate-100">
              {lowAttendance.length ? (
                lowAttendance.map((s, i) => (
                  <div
                    key={s._id}
                    className="flex justify-between items-center px-5 py-3 hover:bg-rose-50/10 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded bg-rose-100 flex items-center justify-center text-rose-600 font-bold text-xs">
                        {s.firstName?.charAt(0)}
                      </div>
                      <div>
                        <div className="font-semibold text-slate-800 text-xs">
                ...
                <div className="px-5 py-8 text-center text-slate-400 text-xs font-medium flex items-center justify-center gap-1.5">
                  <Check className="w-4 h-4 text-emerald-600" /> All students
