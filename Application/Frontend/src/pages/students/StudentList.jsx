...
// ---------- Floating Label Select (view-mode: plain text) ----------
const Select = ({
  label,
  name,
  options = [],
...
const generateStudents = () => {
  ...
  return Array.from({ length: 35 }, (_, i) => {
    ...
    const isNewThisMonth =
      new Date(admissionDate).getMonth() === new Date().getMonth() &&
    ...
    return {
      ...
      class: studentClass,
      section,
      rollNumber: `${studentClass.slice(0, 2)}-${1000 + i}`,
      phone: `03${Math.floor(Math.random() * 9) + 1}${Math.floor(
        Math.random() * 10000000,
      )
        .toString()
        .padStart(7, "0")}`,
      email: `${name.toLowerCase().replace(/\s/g, ".")}@student.pphs.edu.pk`,
      fatherName: `Father of ${name}`,
      motherName: `Mother of ${name}`,
      parentPhone: `03${Math.floor(Math.random() * 9) + 1}${Math.floor(
        Math.random() * 10000000,
      )
        .toString()
        .padStart(7, "0")}`,
      address: `${Math.floor(Math.random() * 500) + 1}, ${["Main St", "Gulberg", "Defence", "Johar Town"][Math.floor(Math.random() * 4)]}, ${cities[Math.floor(Math.random() * cities.length)]}`,
      city: cities[Math.floor(Math.random() * cities.length)],
      dateOfBirth: `200${Math.floor(Math.random() * 8) + 1}-0${Math.floor(Math.random() * 9) + 1}-${Math.floor(Math.random() * 28) + 1}`,
      ...
      religion: Math.random() > 0.5 ? "Islam" : "Christianity",
      nationality: "Pakistani",
      previousSchool: `School ${Math.floor(Math.random() * 10) + 1}`,
      medicalInfo: Math.random() > 0.7 ? "Asthma" : "None",
...
const StudentFormModal = ({ isOpen, onClose, student, mode, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(false);
    onSave(
      { id: student?.id, ...formData, picture: imagePreview },
      profileImage,
    );
    onClose();
  };
  ...
  const SectionHeader = ({ title }) => (
    <h3 className="font-semibold text-slate-700 text-sm border-b border-slate-100 pb-2 mb-3">
      {title}
    </h3>
  );
