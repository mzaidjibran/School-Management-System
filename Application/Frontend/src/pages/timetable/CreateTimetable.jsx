...
const CellEditor = ({ cell, day, slot, subjects, teachers, onChange }) => {
  const selCls =
    "w-full px-2 py-1.5 text-xs border border-slate-200 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-indigo-300";
  return (
    <div className="space-y-1.5">
      <select
        value={cell.subject}
        onChange={(e) => onChange(day, slot, "subject", e.target.value)}
        className={selCls}
      >
      ...
      <select
        value={cell.teacher}
        onChange={(e) => onChange(day, slot, "teacher", e.target.value)}
        className={selCls}
      >
...
export default function CreateTimetable() {
  ...
  useEffect(() => {
    ...
    const filtered = allSubjects.filter((s) => {
      if (Array.isArray(s.class))
        return s.class.some((c) => (c._id || c) === form.classId);
  ...
  const cellSelect =
    "w-full px-1.5 py-1 text-xs border border-slate-200 rounded-md bg-white focus:outline-none focus:ring-1 focus:ring-indigo-300 leading-tight";
        ...
        {!loadingDropdowns && form.classId && (
          <div className="flex flex-wrap gap-3 mt-3">
            <span className="text-xs text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full font-medium">
              ...
              {filteredSubjects.length !== 1 ? "s" : ""} for this class
            </span>
      ...
      <div className="bg-white rounded-md shadow-sm border border-slate-100 overflow-hidden">
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                {DAYS.map((day) => (
                  <th
                    key={day}
                    className="py-2.5 px-2 text-left text-xs font-semibold text-slate-600 min-w-[140px]"
                  >
                    {day}
                  </th>
                ))}
                  ...
                  >
                    ...
                    {DAYS.map((day) => {
                      ...
                      return (
                        <td
                          key={day}
                          className={`py-1.5 px-1.5 align-top ${cell.subject ? "bg-indigo-50/40" : ""}`}
                        >
        ...
        <div className="block md:hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-indigo-50">
            ...
            <button
              onClick={() =>
                setActiveDay((d) => Math.min(DAYS.length - 1, d + 1))
              }
          ...
          <div className="p-4 space-y-3">
            {slots.length === 0 ? (
              ...
              slots.map((slot) => {
                const cell = timetable[DAYS[activeDay]]?.[slot] || {
                  subject: "",
                  teacher: "",
                  room: "",
                };
                return (
                  <div
                    key={slot}
                    className={`rounded-md border p-3 ${cell.subject ? "border-indigo-200 bg-indigo-50/40" : "border-slate-100 bg-white"}`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        onClick={() => removeSlot(slot)}
                        className="text-slate-300 hover:text-rose-400 transition no-print"
                      >
                        <FaTrash className="text-xs" />
                      </button>
                    </div>
