        ...
        <div className="flex justify-between items-center px-5 py-4 border-b border-slate-100 shrink-0">
          <div>
            <h3 className="text-sm font-bold text-slate-800">
              Timetable — {tt.className}
              {tt.section && tt.section !== "—" ? ` / ${tt.section}` : ""}
            </h3>
                  ...
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      {DAYS.map((day) => (
                        <th
                          key={day}
                          className="py-2 px-2 text-left font-semibold text-slate-600 min-w-[90px]"
                        >
                          {day.slice(0, 3)}
                        </th>
                      ))}
                    ...
                    <div className="divide-y divide-slate-50">
                      {DAYS.map((day) => {
                        ...
                        return (
                          ...
                          >
                            <span className="text-[10px] font-semibold text-slate-400 uppercase w-10 shrink-0 mt-0.5">
                              {day.slice(0, 3)}
                            </span>
...
export default function TimetableList() {
  ...
  const handleDuplicate = async (tt) => {
    if (!tt.timetableData.length) {
      toast.error("This class does not have a timetable to duplicate.");
      return;
    }
    try {
      await Promise.all(
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {statCards.map((c) => (
          ...
          >
            ...
            <div
              className={`w-10 h-10 ${c.bg} rounded-md flex items-center justify-center text-lg ${c.ic}`}
            >
              {c.icon}
            </div>
          </div>
          ...
          <>
                ...
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
            ...
            <div className="block sm:hidden p-4 space-y-3">
              {filtered.map((tt, idx) => {
                ...
                const periodCount = tt.timetableData.reduce(
                  (sum, d) => sum + (d.periods?.length || 0),
                  0,
                );
                return (
                  <div
                    key={tt._id}
                    className="border border-slate-100 rounded-md shadow-sm bg-white p-4"
                  >
                    {/* Top */}
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`w-9 h-9 rounded-md flex items-center justify-center font-bold text-sm ${colors[idx % colors.length]}`}
                        >
                      ...
                      {tt.timetableData.length > 0 ? (
                        <span className="px-2 py-0.5 text-[10px] bg-emerald-50 text-emerald-700 rounded-full font-medium">
                          Active
                        </span>
                      ) : (
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <button
                        onClick={() => setViewTimetable(tt)}
                        className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-md transition"
                      >
                        <FaEye /> View
                      </button>
                      <button
                        onClick={() => navigate(`/timetable/${tt._id}`)}
                        className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-md transition"
                      >
                        <FaEdit /> Edit
                      </button>
                      <button
                        onClick={() => handleDuplicate(tt)}
                        className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-md transition"
                      >
                        <FaCopy /> Copy
                      </button>
                      <button
                        onClick={() => handlePrint(tt)}
                        className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-md transition"
                      >
                        <FaPrint /> Print
                      </button>
                      <button
                        onClick={() => handleDelete(tt)}
                        className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-md transition"
                      >
                        <FaTrash /> Del
                      </button>
                    </div>
